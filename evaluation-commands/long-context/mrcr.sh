...
for i in $(seq 1 "$REPS"); do
    ...
    chg run --gpus 1 -- lm_eval --model local-chat-completions \
        --tasks ${TASK_TAG} \
        --model_args "model=$MODEL_NAME,max_length=${MAX_LENGTH},base_url=${BASE_URL}/chat/completions,num_concurrent=128,max_retries=3,tokenized_requests=False,tokenizer_backend=None,timeout=1200,enable_thinking=True,think_end_token=</think>" \
        --num_fewshot $SHOTS \
        --apply_chat_template \
        --fewshot_as_multiturn \
        --output_path results/${TASK_NAME}_${i}_seed_${SEED}.json \
        --seed "$SEED" \
        --gen_kwargs "do_sample=True,temperature=0.6,top_p=0.9,top_k=50,max_gen_toks=${MAX_GEN_TOKS},thinking_token_budget=${MAX_THINK_TOKS},seed=$SEED"
done
