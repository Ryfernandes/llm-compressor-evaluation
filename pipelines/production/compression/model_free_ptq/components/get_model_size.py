...
def get_model_size(
    model_id: str,
    memory_multiplier: int = 3,
    memory_buffer_gi: int = 10,
) -> str:
    """
    ...
    compress_memory_gi = int(math.ceil(model_size_gi * memory_multiplier)) + memory_buffer_gi
    memory_request = f"{compress_memory_gi}Gi"

    print(f"Memory multiplier: {memory_multiplier}x")
    print(f"Memory buffer: {memory_buffer_gi} GiB")
